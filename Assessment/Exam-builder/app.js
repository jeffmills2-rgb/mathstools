...
function renderControlDashboard() {
  ...
  const workedToggleText = showWorkedSolutions ? "Hide Worked Solutions" : "Show Worked Solutions";

  controlsRoot.innerHTML = `
    <section class="builder-dashboard" aria-label="Exam builder controls">
      <div class="builder-dashboard-header">
        <div>
          <h1>CHHS Exam Builder</h1>
          <p>Set exam details, choose topics, generate, edit and print.</p>
        </div>

        <div class="builder-summary">
          <span><strong>${selectedTopicCount}</strong> Stage 4 topics selected</span>
          <span><strong>${selectedQuestionTotal}</strong> extended questions</span>
          <span><strong>${draftConfig.multipleChoiceCount || 0}</strong> multiple choice</span>
        </div>
      </div>

      <div class="builder-control-row">
        <button type="button" class="builder-primary-control" data-control-action="open-exam-details">
          Exam Details
        </button>

        <button type="button" class="builder-primary-control" data-control-action="open-stage-4">
          Select Topics (Stage 4)
        </button>

        <button type="button" class="builder-primary-control is-muted" data-control-action="open-stage-5">
          Select Topics (Stage 5)
        </button>

        <button type="button" class="builder-generate-button" data-control-action="generate-paper">
          ${hasExam ? "Regenerate Paper" : "Generate Paper"}
        </button>
      </div>

      ${controlsStatus ? `<p class="builder-status">${escapeHtml(controlsStatus)}</p>` : ""}

      <div class="builder-print-actions" aria-label="Print and solutions controls">
        <button type="button" class="builder-print-primary" data-control-action="print-student" ${hasExam ? "" : "disabled"}>
          Print / Save Student Exam
        </button>

        <button type="button" data-control-action="toggle-solutions" ${hasExam ? "" : "disabled"}>
          ${workedToggleText}
        </button>

        <button type="button" data-control-action="print-solutions" ${hasExam ? "" : "disabled"}>
