  ...
  return `
    <section class="exam-section ${escapeHtml(extraClass)}">
      ...
      <div class="question-list">
        ${questions.map((question, index) => renderQuestion(question, startNumber + index, options)).join("")}
      </div>
    </section>
  `;
}
...
function renderMultipleChoiceAnswerSheet(exam) {
  const multipleChoiceCount = Number(exam.multipleChoiceCount ?? getMultipleChoiceQuestions(exam).length);
...
export function renderExam(target, exam) {
