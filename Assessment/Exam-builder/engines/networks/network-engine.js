...
(function () {
  ...
  function drawEdge(svg, nodeMap, edge, opts = {}) {
    ...
    const stroke = edge.highlight ? (edge.color || opts.highlightEdge || BLUE) : (edge.color || opts.edgeColor || INK);
    const width = edge.highlight ? (edge.width || 4) : (edge.width || opts.edgeWidth || 3);
    ...
    if (edge.label) {
      const mx = (a.x + b.x) / 2;
      const my = (a.y + b.y) / 2;
      addTextBox(svg, edge.label, mx + (edge.labelDx || 0), my + (edge.labelDy || 0), { size: 15 });
    }
  }
  ...
  function renderDegree(node, config = {}) {
    ...
    graph.nodes.forEach(n => {
      if (n.id === target) {
        n.highlight = true;
        n.caption = `degree ${degreeMap(graph)[target]}`;
      }
    });
    graph.edges = graph.edges.map(edge => {
      const { from, to } = edgeEnds(edge);
      return from === target || to === target ? { from, to, highlight: true, color: BLUE } : edge;
    });
    drawGraph(svg, graph);
    addTitle(svg, `Degree of vertex ${target}`, 360, 42);
    addText(svg, "Count the edges that meet at the vertex.", 360, 372, { size: 16, fill: MUTED, weight: 700 });
    node.appendChild(svg);
  }
  ...
  function renderSameGraphPair(node) {
    ...
    drawGraph(svg, left);
    drawGraph(svg, right);
    addText(svg, "Same vertices and same connections; different layout.", 360, 374, { size: 16, fill: MUTED, weight: 700 });
    node.appendChild(svg);
  }
  ...
  function renderPlanarRedraw(node) {
    ...
    addText(svg, "Crossings in one drawing do not always mean non-planar.", 360, 372, { size: 15, fill: MUTED, weight: 700 });
    node.appendChild(svg);
  }
  ...
  function renderEulerFaces(node, config = {}) {
    ...
    addTextBox(svg, "F₄ is the outside face", 555, 110, { size: 17, fill: MUTED });
    addTextBox(svg, `v = ${config.vertices || 5}, e = ${config.edges || 7}, f = ${config.faces || 4}`, 555, 165, { size: 18, fill: INK });
    addTextBox(svg, "v − e + f = 2", 555, 220, { size: 22, fill: INK, background: "#eef2ff" });
    node.appendChild(svg);
  }
  ...
  function renderConnectedComparison(node) {
    ...
    addText(svg, "Connected", 205, 365, { size: 16, fill: GREEN, weight: 800 });
    addText(svg, "Disconnected", 515, 365, { size: 16, fill: RED, weight: 800 });
    node.appendChild(svg);
  }
  ...
  function renderWalkTrail(node) {
    ...
    addText(svg, "A path has no repeated vertices; a trail has no repeated edges.", 360, 372, { size: 16, fill: MUTED, weight: 700 });
    node.appendChild(svg);
  }
  ...
  function renderEulerian(node, config = {}) {
    ...
    graph.nodes = graph.nodes.map(n => deg[n.id] % 2 === 1
      ? { ...n, highlight: true, caption: `degree ${deg[n.id]}` }
      : { ...n, caption: `degree ${deg[n.id]}` });
    drawGraph(svg, graph);
    const oddCount = Object.values(deg).filter(d => d % 2 === 1).length;
    const summary = oddCount === 0
      ? "0 odd vertices → Eulerian circuit"
      : oddCount === 2
        ? "2 odd vertices → Eulerian trail"
        : `${oddCount} odd vertices → no Eulerian trail`;
    addTitle(svg, summary, 360, 45, { fill: oddCount > 2 ? RED : GREEN, size: 18 });
    node.appendChild(svg);
  }

  function renderKonigsberg(node) {
    const svg = makeSvg();
    // Stylised river and land regions.
    svg.appendChild(svgEl("rect", { x: 55, y: 75, width: 300, height: 260, rx: 24, fill: RIVER, stroke: "none" }));
    addPath(svg, "M195 75 C175 130 178 210 198 335", { stroke: "#93c5fd", width: 38, fill: "none" });
    addPath(svg, "M55 210 C140 192 240 205 355 185", { stroke: "#93c5fd", width: 38, fill: "none" });
    addPoly(svg, [[92, 120], [175, 110], [168, 185], [82, 178]], { fill: LAND, stroke: "#c084fc", width: 2 });
    addPoly(svg, [[235, 105], [320, 120], [310, 178], [230, 165]], { fill: LAND, stroke: "#c084fc", width: 2 });
    addPoly(svg, [[95, 250], [178, 235], [178, 308], [88, 302]], { fill: LAND, stroke: "#c084fc", width: 2 });
    addPoly(svg, [[230, 230], [315, 222], [322, 305], [238, 315]], { fill: LAND, stroke: "#c084fc", width: 2 });
    addPoly(svg, [[175, 175], [232, 168], [236, 235], [180, 242]], { fill: LAND, stroke: "#fde68a", width: 2 });

    const bridges = [
      [165, 145, 220, 135], [168, 170, 226, 160], [170, 260, 230, 248],
      [170, 285, 235, 276], [124, 190, 180, 203], [232, 202, 298, 190], [205, 230, 208, 172]
    ];
    bridges.forEach(([x1, y1, x2, y2]) => addLine(svg, x1, y1, x2, y2, { stroke: "#92400e", width: 5 }));
    addTitle(svg, "Königsberg bridges", 205, 50, { size: 17 });
    ...
    drawGraph(svg, graph);
    addText(svg, "4 odd vertices", 535, 360, { size: 18, fill: RED, weight: 800 });
    node.appendChild(svg);
  }
