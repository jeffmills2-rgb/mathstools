...
window.MMT_LENGTH_ENGINE = (() => {
  ...
  function makeSvg(width = 720, height = 420, label = "length diagram") {
    ...
    style.textContent = `
      ...
      .shape-line{stroke:#111827;stroke-width:3.2;stroke-linejoin:round;stroke-linecap:round}
      .shape-dash{stroke:#111827;stroke-width:2.2;stroke-dasharray:7 6;stroke-linecap:round}
      .shape-highlight{stroke:#9f174a;stroke-width:4;fill:none;stroke-linecap:round;stroke-linejoin:round}
      .shape-feature{stroke:#1d4ed8;stroke-width:4;fill:none;stroke-linecap:round;stroke-linejoin:round}
      .shape-feature-green{stroke:#047857;stroke-width:4;fill:none;stroke-linecap:round;stroke-linejoin:round}
      .shape-feature-fill{fill:rgba(29,78,216,.12);stroke:#1d4ed8;stroke-width:3}
      .shape-label{font-family:"Cambria Math","Times New Roman",serif;font-size:22px;font-weight:800;fill:#111827;text-anchor:middle;dominant-baseline:middle}
      .shape-small-label{font-family:"Cambria Math","Times New Roman",serif;font-size:18px;font-weight:700;fill:#111827;text-anchor:middle;dominant-baseline:middle}
      ...
      .missing-label{font-family:"Cambria Math","Times New Roman",serif;font-size:22px;font-weight:900;fill:#9f174a;text-anchor:middle;dominant-baseline:middle}
      .missing-edge{stroke:#9f174a;stroke-width:5;stroke-linecap:round;stroke-linejoin:round}
    `;
  ...
  function renderPolygon(target, config = {}) {
    ...
    if (shape === "rhombus") {
      ...
      drawTickMark(svg, points[1], points[2], 1);
  ...
  function renderSector(target, config = {}) {
    const svg = makeSvg(720, 430, "sector diagram");
    const cx = 360;
    const cy = 230;
    const r = 130;
    ...
    svg.appendChild(el("path", {
      d: arcPath(cx, cy, 38, startAngle, endAngle),
      class: "shape-feature-green"
    }));

    label(svg, `${angle}°`, cx, cy - 58, "shape-small-label");

    if (angle >= 170 && angle <= 190) {
      label(svg, `${radius} ${unit}`, cx + r / 2, cy + 30);
    } else {
      label(svg, `${radius} ${unit}`, (cx + a.x) / 2 - 24, (cy + a.y) / 2);
    }
  ...
  function renderCurvedComposite(target, config = {}) {
    ...
    if (shape === "square-quadrant") {
      const x = 215;
      const y = 100;
      const size = 260;
      const radius = config.side ?? config.radius ?? "";

      svg.appendChild(el("path", {
        d: `M ${x} ${y} H ${x + size} V ${y + size} H ${x} V ${y} Z`,
        class: "shape-fill shape-line"
      }));
      ...
      label(svg, `${radius} ${unit}`, x + size / 2, y + size + 32);
      label(svg, `${radius} ${unit}`, x - 42, y + size / 2);
    } else {
      const x = 170;
      const y = 130;
      const w = 300;
      ...
      label(svg, `${width} ${unit}`, x + w / 2, y - 30);
      label(svg, `${height} ${unit}`, x - 44, y + h / 2);
    }
