...
function unitaryRatioMethodQuestion() {
  ...
  return ratiosRatesQuestion({
    ...
    marks: 2,
    prompt: `A drink is mixed in the ratio cordial:water = ${cordial}:${water}. If ${givenWater} cups of water are used, how many cups of cordial are needed?`,
    answer: `${answer} cups`,
    working: [
...
function realLifeRatioProblemsQuestion() {
  if (Math.random() < 0.5) {
    ...
    return ratiosRatesQuestion({
      ...
      marks: 2,
      prompt: `A map uses a scale of 1:${scale}. A distance on the map is ${mapCm} cm. Find the real distance in kilometres.`,
  ...
  return ratiosRatesQuestion({
    type: "real-life-ratio-problems",
    marks: 2,
    prompt: `A recipe uses flour:sugar in the ratio ${flour}:${sugar}. If ${flourUsed} cups of flour are used, how many cups of sugar are needed?`,
...
function bestBuyRatesQuestion() {
  const unitPrice = choice([1.2, 1.5, 1.8, 2.0, 2.4]);
  const unitsA = choice([4, 5, 6, 8]);
  const unitsB = choice([8, 10, 12, 15]);
  const priceA = Number((unitPrice * unitsA + choice([-0.2, 0, 0.4, 0.8])).toFixed(2));
  const priceB = Number((unitPrice * unitsB + choice([-1.0, -0.4, 0.2, 0.9])).toFixed(2));
  const perA = priceA / unitsA;
  const perB = priceB / unitsB;
  const best = perA < perB ? "Pack A" : "Pack B";

  return ratiosRatesQuestion({
    type: "best-buy-rates",
    marks: 2,
    prompt: `Which is better value? Pack A: ${unitsA} items for ${money(priceA)} or Pack B: ${unitsB} items for ${money(priceB)}.`,
    answer: best,
    working: [
      `Pack A: ${money(priceA)} ÷ ${unitsA} = ${money(perA)} per item.`,
      `Pack B: ${money(priceB)} ÷ ${unitsB} = ${money(perB)} per item.`,
...
function speedDistanceTimeQuestion() {
  ...
  if (mode === "speed") {
    ...
    return ratiosRatesQuestion({
      ...
      marks: 2,
      prompt: `A cyclist travels ${distance} km in ${time} hours. Find the average speed.`,
      answer: `${speed} km/h`,
  ...
  if (mode === "distance") {
    ...
    return ratiosRatesQuestion({
      type: "speed-distance-time",
      marks: 2,
      prompt: `A car travels at ${speed} km/h for ${time} hours. How far does it travel?`,
      answer: `${distance} km`,
  ...
  return ratiosRatesQuestion({
    type: "speed-distance-time",
    marks: 2,
    prompt: `A bus travels ${distance} km at ${speed} km/h. How long does it take?`,
    answer: `${time} hours`,
...
function distanceTimeReadQuestion() {
  ...
  return ratiosRatesQuestion({
    ...
    marks: 1,
    prompt: `Use the distance-time graph to find how far from home the person was after ${point.x} minutes.`,
...
function distanceTimeSpeedQuestion() {
  ...
  return ratiosRatesQuestion({
    ...
    marks: 2,
    prompt: `Use the distance-time graph to find the average speed between ${seg.a.x} and ${seg.b.x} minutes.`,
    diagram: distanceTimeDiagram(graph),
...
function distanceTimeInterpretQuestion() {
  ...
  const seg = choice(flatSegments);

  return ratiosRatesQuestion({
    type: "distance-time-interpret",
    marks: 1,
    prompt: `Use the distance-time graph. During which time interval was the person stopped?`,
    diagram: distanceTimeDiagram(graph),
    answer: `${seg.a.x} to ${seg.b.x} minutes`,
    working: [`A horizontal line means the distance did not change.`, `The person was stopped from ${seg.a.x} to ${seg.b.x} minutes.`],
    space: SPACE_SIZES.SMALL
  });
}
...
function distanceTimeConstructQuestion() {
  ...
  return ratiosRatesQuestion({
    ...
    marks: 2,
    prompt: `Construct a distance-time graph for this journey: ${graph.description}.`,
