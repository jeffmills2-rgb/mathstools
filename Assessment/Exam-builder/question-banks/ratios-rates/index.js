...
function unitaryRatioMethodQuestion() {
  ...
  return ratiosRatesQuestion({
    ...
    marks: 2,
    prompt: `A drink is mixed in the ratio cordial:water = ${cordial}:${water}.\n${PROMPT_INDENT}If ${givenWater} cups of water are used, how many cups of cordial are needed?`,
    answer: `${answer} cups`,
    working: [
...
function realLifeRatioProblemsQuestion() {
  if (Math.random() < 0.5) {
    ...
    return ratiosRatesQuestion({
      ...
      marks: 2,
      prompt: `A map uses a scale of 1:${scale}.\n${PROMPT_INDENT}A distance on the map is ${mapCm} cm. Find the real distance in kilometres.`,
  ...
  return ratiosRatesQuestion({
    type: "real-life-ratio-problems",
    marks: 2,
    prompt: `A recipe uses flour:sugar in the ratio ${flour}:${sugar}.\n${PROMPT_INDENT}If ${flourUsed} cups of flour are used, how many cups of sugar are needed?`,
...
function distanceTimeSpeedQuestion() {
  ...
  return ratiosRatesQuestion({
    ...
    marks: 2,
    prompt: `Use the distance-time graph below to answer the question below.\n${PROMPT_INDENT}Find the average speed between ${seg.a.x} and ${seg.b.x} minutes.`,
    diagram: distanceTimeDiagram(graph),
...
function distanceTimeInterpretQuestion() {
  ...
  return ratiosRatesQuestion({
    ...
    marks: plural ? 2 : 1,
    prompt: `Use the distance-time graph below to answer the question below.\n${PROMPT_INDENT}${plural ? "During which time intervals was the person stopped?" : "During which time interval was the person stopped?"}`,
    diagram: distanceTimeDiagram(graph),
