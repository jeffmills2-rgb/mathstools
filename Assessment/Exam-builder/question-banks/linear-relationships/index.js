...
const TYPE_LIST = [
  ...
  { id: "table-to-coordinates", label: "Tables and coordinate pairs" },
  { id: "graph-from-table", label: "Graph a line from a table" },
  ...
  { id: "verify-intersection", label: "Verify an intersection point" }
];
...
function tableToCoordinatesQuestion() {
  ...
  return linearQuestion({
    type: "table-to-coordinates",
    marks: 1,
    prompt: `The table follows ${lineExpr(m, b)}. Write the matching coordinate pairs.`,
    table: valuesTable(xs, ys, false),
    answer: pairs.map(p => coord(p.x, p.y)).join(", "),
    working: pairs.map(p => `x = ${fmt(p.x)} gives y = ${fmt(p.y)}, so ${coord(p.x, p.y)}.`),
    space: SPACE_SIZES.SMALL
  });
}
...
function graphFromTableQuestion() {
  ...
  return linearQuestion({
    ...
    marks: 2,
    prompt: `Graph the points from this table and draw the straight line.`,
    table: valuesTable(xs, ys, false),
    ...
    working: [`Plot the coordinate pairs: ${pairs.map(p => coord(p.x, p.y)).join(", ")}.`, `Join the points with a straight line.`],
    space: SPACE_SIZES.NONE
  });
}
...
function compareLinearGraphsQuestion() {
  ...
  return linearQuestion({
    ...
    marks: 1,
    prompt: `Which line is steeper, Line A or Line B?`,
    diagram: linearGraphDiagram({
      mode: "completed",
      lines,
      showIntersection: false,
    ...
    answer: steeper,
    working: [`The steeper line has the greater absolute gradient.`, `${steeper} is steeper.`],
    space: SPACE_SIZES.SMALL
  });
}
...
function realLifeLinearQuestion() {
  ...
  return linearQuestion({
    ...
    marks: 1,
    prompt: `A taxi fare costs $${start} plus $${rate} per kilometre. Write a rule for the cost C after k kilometres.`,
...
function makeIntersectingLines() {
  const x = randInt(-3, 4);
  const y = randInt(-3, 6);
  const m1 = choice([-2, -1, 1, 2, 3]);
  let m2 = choice([-3, -2, -1, 1, 2]);
  while (m2 === m1) m2 = choice([-3, -2, -1, 1, 2]);
  ...
  return {
    intersection: { x, y },
    lines: [
      { m: m1, b: b1, label: "Line A" },
      { m: m2, b: b2, label: "Line B" }
    ]
  };
}
...
function verifyIntersectionQuestion() {
  ...
  return linearQuestion({
    ...
    marks: 2,
    prompt: `Verify that ${coord(p.x, p.y)} is the point of intersection for:
${lineExpr(made.lines[0].m, made.lines[0].b)}
${lineExpr(made.lines[1].m, made.lines[1].b)}.`,
    diagram: linearGraphDiagram({
      mode: "completed",
    ...
    working: [
      `Substitute x = ${fmt(p.x)} into ${lineExpr(made.lines[0].m, made.lines[0].b)}: y = ${fmt(p.y)}.`,
      `Substitute x = ${fmt(p.x)} into ${lineExpr(made.lines[1].m, made.lines[1].b)}: y = ${fmt(p.y)}.`,
...
const GENERATORS = {
  ...
  "table-to-coordinates": tableToCoordinatesQuestion,
