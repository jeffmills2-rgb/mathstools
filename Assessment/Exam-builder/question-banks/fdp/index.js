...
function fractionOperationsQuestion() {
  ...
  return fdpQuestion({
    ...
    marks: 2,
    prompt: `Calculate: ${frac(a.n, a.d)} ${op} ${frac(b.n, b.d)}`,
    answer: frac(n, d),
    working,
...
function discountsQuestion() {
  ...
  const unitsB = randInt(8, 14);
  const unitPrice = randInt(120, 260) / 100;
  const priceA = Number((unitsA * unitPrice + randInt(0, 120) / 100).toFixed(2));
  const priceB = Number((unitsB * unitPrice + randInt(-120, 80) / 100).toFixed(2));
  ...
  return fdpQuestion({
    ...
    answer: best,
    working: [`Pack A: ${money(priceA)} ÷ ${unitsA} = ${money(perA)} per item`, `Pack B: ${money(priceB)} ÷ ${unitsB} = ${money(perB)} per item`, `${best} is better value.`],
    space: SPACE_SIZES.MEDIUM
  });
}
